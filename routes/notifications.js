module.exports = function({ app, dbConn }) {
    app.post('/notifications/create', (req, res) => {
        ...
        if (!notificationImage || !notificationMessage || !userId) {
            res.status(500).jsonp({ message: 'Cannot creat the notification, please try again' });
        }
        const createNotification = [
            [notificationImage, notificationMessage, userId]
        ];
    ...
    app.get('/notifications/:id', (req, res) => {
        ...
        dbConn.query(getNotificationsSql, [userId], function(error, notifications) {
            if (notifications) {
                ...
                res.status(500).jsonp({ message: 'Cannot get your notifications, please try again' });
            }
        });
    });
}