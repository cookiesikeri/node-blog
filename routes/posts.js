module.exports = function({ app, dbConn, upload }) {
    app.post('/posts', upload.single('post_image'), (req, res) => {
        ...
        if (!file) {
            ...
            if (postCreatedBy) {
                const createdPost = [
                    [postContent, postCategory, postCreatedDate, postCreatedBy]
                ];
                ...
                dbConn.query(createPostSql, [createdPost], function(error, insertedPost) {
                    if (insertedPost) {
                        ...
                        res.status(412).jsonp({ message: 'Cannot upload your post, please try again' });
                    }
                });
            } else {
                res.status(500).jsonp({ message: 'Cannot upload your post, please try again' });
            }
        }
    });
    ...
    app.get('/posts', (req, res) => {
        ...
        dbConn.query(getPostsSql, function(error, posts) {
            if (posts) {
                ...
                res.status(500).jsonp({ message: 'Cannot get your posts, please try again' });
            }
        });
    });
    ...
    app.post('/posts/reactions', (req, res) => {
        ...
        dbConn.query(updateNumberOfReactionsSql, [numberOfReactions, id], function(err, updatedPost) {
            if (err) {
                res.status(500).jsonp({ message: "The system error. Please try again" });
            } else if (updatedPost) {
