module.exports = function({ app, dbConn }) {
    ...
    app.post('/reactions/create', (req, res) => {
        ...
        if (!postId || !userId) {
            res.status(500).jsonp({ message: 'Cannot create the post reaction, please try again' });
        }
        const reactions = [
            [postId, userId]
        ];
    ...
    app.post('/reactions/delete', (req, res) => {
        ...
        dbConn.query(deleteReactionsSql, [postId, userId], function(error, response) {
            if (response && response.affectedRows) {
                ...
                res.status(500).jsonp({ message: 'Cannot delete the post reaction, please try again' });
            }
        });
    });
}